...
from .utils import (
    ...
    sync_maternity_discharge_from_ipd,
...
def discharge_patient(request, admission_id):
    ...
    admission = get_object_or_404(Admission, id=admission_id)
    maternity_delivery = getattr(admission, 'delivery', None)
    is_maternity_admission = maternity_delivery is not None
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and formset.is_valid():
            ...
            if has_meds:
                ...
                if prescription_items:
                    ...
                    take_home_meds_text = '\n'.join(
                        f"- {item.medication.name} x{item.quantity}"
                        for item in prescription_items
                    )
            ...
            if is_maternity_admission:
                sync_maternity_discharge_from_ipd(
                    admission,
                    discharge,
                    request.user,
                    medications_prescribed=take_home_meds_text,
                )
        ...
        if is_maternity_admission:
            delivery_notes = maternity_delivery.delivery_notes or ''
            if maternity_delivery.get_delivery_mode_display():
                delivery_notes = (
                    f"Delivery: {maternity_delivery.get_delivery_mode_display()}\n"
                    f"{delivery_notes}"
                ).strip()
            discharge_initial['final_diagnosis'] = (
                maternity_delivery.get_delivery_mode_display()
                or admission.provisional_diagnosis
                or ''
            )
            discharge_initial['clinical_management_summary'] = (
                f"{management_summary}\n\nMATERNITY EPISODE:\n{delivery_notes}".strip()
            )

        # Pre-fill provisional diagnosis from admission and autogenerated procedures
        form = InpatientDischargeForm(instance=discharge_instance, initial=discharge_initial)
    ...
    maternity_pregnancy = maternity_delivery.pregnancy if is_maternity_admission else None

    return render(request, 'inpatient/discharge_patient.html', {
        'admission': admission,
        'form': form,
        'formset': formset,
        'discharge_instance': discharge_instance,
        'med_json': med_json,
        ...
        'is_maternity_admission': is_maternity_admission,
        'maternity_delivery': maternity_delivery,
...
def _can_view_clean_admissions(user):
    return user.is_authenticated and (
        user.is_superuser or user.role in ('Admin', 'SHA Manager')
    )


@login_required
def clean_admissions(request):
    """List open IPD admissions and orphan active inpatient visits (Admin / SHA Manager)."""
    if not _can_view_clean_admissions(request.user):
        messages.error(request, 'Only Admin and SHA Manager can access Clean Admissions.')
    ...
    active_admissions = (
        ...
        .annotate(
            is_maternity=Exists(
                LaborDelivery.objects.filter(admission_id=OuterRef('pk'))
            ),
        )
        .select_related('patient', 'visit', 'visit__invoice', 'bed', 'bed__ward', 'delivery__pregnancy')
        .order_by('-admitted_at')
    )
    ...
    for adm in active_admissions:
        ...
        if adm.is_maternity:
            try:
                pregnancy_id = adm.delivery.pregnancy_id
        ...
        admission_rows.append({
            ...
            'visit_date': adm.visit.visit_date if adm.visit else adm.admitted_at,
            'admitted_at': adm.admitted_at,
            'invoice_amount': invoice.total_amount if invoice else None,
            'invoice_status': invoice.status if invoice else None,
            'invoice_id': invoice.id if invoice else None,
            'is_maternity': adm.is_maternity,
            'admission_id': adm.id,
            'pregnancy_id': pregnancy_id,
            'ward': adm.bed.ward.name if adm.bed else None,
            'bed': adm.bed.bed_number if adm.bed else None,
        })

    admitted_visit_ids = active_admissions.values_list('visit_id', flat=True)
    orphan_visits = (
        Visit.objects.filter(visit_type='IN-PATIENT', is_active=True)
    ...
    for visit in orphan_visits:
        ...
        orphan_rows.append({
            ...
            'invoice_status': invoice.status if invoice else None,
            'invoice_id': invoice.id if invoice else None,
            'is_maternity': LaborDelivery.objects.filter(visit_id=visit.id).exists(),
            'admission_id': None,
            'pregnancy_id': None,
            'ward': None,
            'bed': None,
    ...
    all_rows = admission_rows + orphan_rows
    all_rows.sort(key=lambda r: r['visit_date'] or timezone.now(), reverse=True)

    maternity_count = sum(1 for r in admission_rows if r['is_maternity'])
    normal_count = len(admission_rows) - maternity_count

    return render(request, 'inpatient/clean_admissions.html', {
        'rows': all_rows,
        'total_admissions': len(admission_rows),
        'orphan_visit_count': len(orphan_rows),
        'maternity_count': maternity_count,
        'normal_count': normal_count,
        'title': 'Clean Admissions',
    })


@login_required
@require_POST
def clean_admissions_close(request):
    """Close an open admission + visit, or an orphan IPD visit, from Clean Admissions."""
    if not _can_view_clean_admissions(request.user):
        messages.error(request, 'Only Admin and SHA Manager can close admissions.')
        return redirect('users:dashboard')
    ...
    if admission_id:
        admission = get_object_or_404(Admission, pk=admission_id, status='Admitted')
        patient_name = admission.patient.full_name
        admin_close_admission_and_visit(admission, request.user)
        messages.success(
            request,
            f'Admission and visit closed for {patient_name}.',
        )
    elif visit_id:
        ...
        if Admission.objects.filter(visit=visit, status='Admitted').exists():
            messages.error(request, 'This visit still has an open admission. Close the admission instead.')
            return redirect('inpatient:clean_admissions')
        patient_name = visit.patient.full_name
        admin_close_orphan_visit(visit)
        messages.success(request, f'Visit #{visit.id} closed for {patient_name}.')
    else:
